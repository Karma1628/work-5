...
class UniAD(nn.Module):
    def __init__(
        ...
        neighbor_mask={'mask': [True, True, True], 'neighbor_size': [7, 7]}, # default [7, 7]
        hidden_dim=256,
    ...
    def forward(self, input):
        ...
        output_decoder, output_encoder = self.transformer(
        ...
        latent = rearrange(output_encoder, "(h w) b c -> b c h w", h=self.feature_size[0])
        
        return feature_rec, normalize(latent)
...
class Transformer(nn.Module):
    def __init__(
        ...
        hidden_dim=256,
        feature_size=[14, 14],
        neighbor_mask={'mask': [True, True, True],'neighbor_size': [7, 7]},
        nhead=8,
...
if __name__ == '__main__':
    ...
    x = torch.randn(4, 384, 16, 16).cuda()
    model = UniAD(
        inplanes=384,
        instrides=16,
        feature_size=[16, 16],
